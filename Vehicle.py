...
from sql import loadVehicle, add_to_table3, Delete3, Update3
...
class VehicleFrame(tk.Frame):
    def __init__(self, parent,controller):
        ...
        labelID = tk.Label(InnerFrameVehicle1Ver2,bg="#2D2B55", text="Customer ID",fg="#A599E9")
        labelID.grid(row=0, column=0, padx=0, pady=10, sticky="e")

        Enter_ID = tk.Entry(InnerFrameVehicle1Ver2, width=20, textvariable=vehicleid_var,relief="flat",bg="#A599E9")
        Enter_ID.grid(row=0, column=1, padx=0, pady=10) 

        labelType = tk.Label(InnerFrameVehicle1Ver2,bg="#2D2B55", text="Name",fg="#A599E9") 
        labelType.grid(row=1, column=0, padx=0, pady=10, sticky="e")
        
        Enter_Type = tk.Entry(InnerFrameVehicle1Ver2, width=20, textvariable=type_var,relief="flat",bg="#A599E9")
        Enter_Type.grid(row=1, column=1, padx=0, pady=10)

        labelMake = tk.Label(InnerFrameVehicle1Ver2,bg="#2D2B55", text="Email",fg="#A599E9") 
        labelMake.grid(row=2, column=0, padx=0, pady=10, sticky="e")
        
        Enter_Make = tk.Entry(InnerFrameVehicle1Ver2, width=20, textvariable=make_var,relief="flat",bg="#A599E9")
        Enter_Make.grid(row=2, column=1, padx=0, pady=10)

        labelModel = tk.Label(InnerFrameVehicle1Ver2,bg="#2D2B55", text="Phone",fg="#A599E9")
        labelModel.grid(row=3, column=0, padx=0, pady=10, sticky="e")
        
        Enter_Model = tk.Entry(InnerFrameVehicle1Ver2, width=20, textvariable=model_var,relief="flat",bg="#A599E9")
        Enter_Model.grid(row=3, column=1, padx=0, pady=10)

        labeltotalcost = tk.Label(InnerFrameVehicle1Ver2,bg="#2D2B55", text="Amount",fg="#A599E9") 
        labeltotalcost.grid(row=4, column=0, padx=0, pady=10, sticky="e")
        
        Enter_totalcost = tk.Entry(InnerFrameVehicle1Ver2, width=20, textvariable=totalcost_var,relief="flat",bg="#A599E9")
        ...
        def add():
            id = Enter_ID.get()
            types = Enter_Type.get()
            make = Enter_Make.get()
            model = Enter_Model.get()
            totalcost = Enter_totalcost.get()
            add_to_table3(tableVehicle,id,types,make,model,totalcost)
            DeleteEntry()
        def update():
        ...
        AddButton = tk.Button(InnerFrameVehicle1Ver3, text="Add",width=10,command=add,bg="#A599E9")
